...
class TestTolerance {
  ...
  def Test1_clearAccentToMaj(): Unit = {
    assertEquals(
      List("OU", "EST", "LA", "MAIRIE"),
      FautesImpl.clearAccentToMaj(List("Où", "est", "LA", "mâirie"))
    )
  }
  @Test
  ...
  def Test1_correction(): Unit = {
    assertEquals(
      List("Ou", "est", "la", "Mairie"),
      FautesImpl.correction(listeTest, motsBDD)
    )
  }
  @Test
  ...
  def Test4_correction(): Unit = {
    assertEquals(
      List("Mairie", "Gare", "Gare"),
      FautesImpl.correction(List("mairie", "gare", "gae"), motsBDD)
    )
  }
  @Test
  def Test5_correction(): Unit = {
    assertEquals(
      List("TNB", "TNB", "Hôtel de Ville"),
